...
class PreactionCopyPassportAddress(BasePreActionProcessor):
    async def pre_action_processor(
        ...
        if (
            not res_addr
            or res_addr.same_as_passport_address != SAMEASPASSADDR.SAME_AS_PASS_ADDR
        ):
            return payload
        ...
        data.setdefault("residential_address", {})[
            "residential_address_card_v2"
        ] = new_card.model_dump()
